...
**Global verdict: `HF_DEMENTI` (denied) — 0/6 models**, under thresholds frozen before
measurement (commit [`aa120bd`](https://github.com/SimonBouhier/Origami_Transformer/commit/aa120bd63b462fc1f40ab5e0812db5355999c641),
stamped by [`d89a45f`](https://github.com/SimonBouhier/Origami_Transformer/commit/d89a45fb736448d73a796e55f4ece8d233bff7d3)). The 48/48 GPU measurements match
...
The campaign script requires Bash, a CUDA-capable GPU, and a Windows Python
virtual environment at `.venv-gpu/`: it invokes
`./.venv-gpu/Scripts/python.exe` directly. Install the dependencies from
[`requirements.txt`](requirements.txt) in that environment with a CUDA-enabled
PyTorch build. The `python` used for the analysis commands also needs the
repository dependencies. These commands rerun the research campaign and analysis.

```bash
# corpus integrity is re-checked against the frozen sha256 at run time
bash run_campaign_v7.sh   # 6 models x 4 arms x 2 modes = 48 GPU measurements
...
The [v5 corpus construction brief](corpora/README_corpora.md) retains its
pre-freeze instructions as a historical record. Read it alongside the completed
[v5 results](NOTE_RESULTATS_v5.md), not as a current instruction to edit the corpus.
The state-of-play snapshot in `CLAUDE.md` is also historical; use
[`RESEARCH_LOG.md`](RESEARCH_LOG.md) and the result notes for subsequent decisions.
...
[docs](https://epp-verdict-docs.vercel.app)). The Triptique overview
